    ...
    const docs = {
            ...
            'fhir.build': {
                ...
                useCases: [
                    ...
                    'Set rowsPath to build one resource per row from an array (e.g. one Claim per line-item, one Patient per claim context) without a control.loop step — the array of built resources plugs straight into a Payload Builder (FHIR Bundle mode) resourcePaths entry alongside single-resource paths.',
                    'Gate a field, a whole repeating group, or an entire per-row resource with condition/groupCondition instead of a separate upstream if_then_else step, when the condition is naturally scoped to this one resource\'s own fields.',
                ],
                example: {
                ...
                parameters: [
                    ...
                    { name: 'rowsPath', type: 'string', required: false, description: 'When set, switches this step from building ONE resource out of the whole input to building ONE RESOURCE PER ROW found at this path (relative to inputData), writing an array to outputField instead of a single object. This is what makes N independent resources of the same type (e.g. one Claim per row in a multi-claim EDI file, or one Patient per CSV row) possible with a single fhir.build step — no control.loop step-ID wiring needed. Every field/repeatingGroups sourcePath is resolved ROW-SCOPED when rowsPath is set (relative to that one row, the same convention repeatingGroups.fields already use) — there is no automatic top-level fallback for a plain sourcePath in this mode, only condition/groupCondition still merge in topLevel/inputData.' },
                    { name: 'fields', type: 'array', required: false, description: 'Flat/single-value element mappings: {targetPath, sourcePath, fallbackPaths?, literalValue?, transform?, valueMap?, condition?}. targetPath is relative to the resource root (e.g. "birthDate", or a manually-indexed "identifier[0].value"). sourcePath should be "message.<field>" to read the inbound message (or, in rowsPath mode, a field name relative to the current row). condition, when set, gates whether this one field is written at all — evaluated per-row when rowsPath is set.' },
                    { name: 'repeatingGroups', type: 'array', required: false, description: 'One entry per repeating element: {targetPath, rowsPath, fields: [...], condition?, groupCondition?}. rowsPath points to the array of source rows (e.g. "message.patientIdentifiers"); one sub-object is built per row (keeping multiple fields from the same row aligned), written at targetPath[0], targetPath[1], .... Each field\'s own sourcePath resolves relative to one row, not the pipeline root. condition is evaluated once per resolved row (skips just that row); groupCondition is evaluated ONCE against the parent row/inputData BEFORE rowsPath is even resolved (skips the whole group, rowsPath never resolved, targetPath never written) — the two answer different questions ("does this one row qualify" vs. "should this group be attempted at all") and can be combined for both checks together.' },
                ],
                troubleshooting: [
                    {
                    ...
                    {
                        issue: 'I need "only populate this field, or this whole repeating group, when some other field has a given value" — is there a condition option?',
                        cause: 'Yes — a field entry\'s condition and a repeatingGroups entry\'s condition/groupCondition cover this directly now (ported from hl7.build\'s own Condition mechanism). Reach for this before an upstream if_then_else step when the check is naturally scoped to this one resource\'s own fields.',
                        fix: 'Field-level gating: {targetPath: "maritalStatus", sourcePath: "message.maritalStatus", condition: {field: "message.maritalStatusKnown", operator: "equals", value: true}}. Whole-group gating (e.g. only build a Coverage.subscriber reference when the patient IS the subscriber): repeatingGroups entry with groupCondition: {field: "patientInfo.relationshipCode", operator: "equals", value: "18"}. For genuinely multi-step branching logic that spans more than this one resource, an upstream if_then_else/switch_case step (writing a resolved message.* field this step then reads with an ordinary sourcePath) is still the better fit — the two mechanisms are complementary, not either/or.',
                    },
                    {
                        issue: 'What is _rowIndex and where does it come from?',
                        cause: 'Not an error — a synthetic field. When rowsPath is set (top-level or on a repeatingGroups entry), this step stamps a 1-based "_rowIndex" onto every row (as a string, like every other resolved field) BEFORE field/repeatingGroups resolution runs, giving every row a stable ordinal a field mapping can reference.',
                        fix: 'Reference it like any other row field, e.g. {targetPath: "item.sequence", sourcePath: "_rowIndex", transform: "cda_decimal_string_to_number"} — this is exactly how EOB/Claim item.sequence numbers are built from a rowsPath-driven fhir.build step.',
                    },
                    {
                ...
                stepOutput: {
                    description: 'Available to every step after this one.',
                    fields: [
                        { name: 'fhirResource', type: 'object | array', description: 'FHIR R4 resource built from configured field mappings, ready for a Payload Builder (FHIR Bundle mode) step or direct delivery. When rowsPath is set, this is instead an ARRAY of resources (one per row) — the same field name, just array-shaped; Payload Builder\'s resourcePaths already accepts either shape mixed in the same list.' },
                    ],
                },
            },
            'hl7.build': {
