...
| `interpretationCode` → `interpretation` | CONF:4537-7147 | `Observation.interpretation` | inline, reads from COMP entryRelationship only | `observation_mapper.go:241-255` | MEDIUM | Kareo: `interpretationCode` present as a **direct sibling element**, not nested in COMP | **Likely gap**: Go's structural assumption (COMP-nested) doesn't match the IG-correct (and corpus-observed) direct-child shape — would silently miss this field on real data |
