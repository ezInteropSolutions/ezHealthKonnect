...
## NCPDP Telecommunication D.0 — B2 (Reversal), B3 (Rebill), E1 (Eligibility Verification) (September 2026)

### Why these three, and the sourcing bar applied
Closes 3 of D.0 Phase 1's own named-deferred transaction codes. The user's own explicit direction for this round was "no guesswork, only documented verified work" — every prior sourcing attempt for D.0's remaining codes (E1/E2, D0/D1 Prior Authorization, the batch standard) via general web search only produced vague, marketing-level paraphrases, not exact byte/field-level detail. A real, official, freely-published payer sheet closed that gap for exactly 3 codes: **State of North Dakota Medicaid's own NCPDP D.0 payer sheet** (`b1-b2-b3-E1-payer-sheet-exclusion.pdf`, hhs.nd.gov), a single combined document covering B1 (already built), B2, B3, and E1 end to end — request AND response, every segment/field table, Mandatory/Required/RW status all explicit. Downloaded and read via `pdftotext -layout` directly (WebFetch's own AI-summarized read of the same PDF silently dropped structural detail — a real, repeat instance of the already-documented "AI-summarized re-transcription is a fidelity risk for exact-byte-value data" lesson from the 837 work) — every field/segment claim below is a direct line-by-line read of the extracted text, not a paraphrase.

**E2, D0/D1 (Prior Authorization), N1-N3, C1-C3, S1-S3, and the batch standard (STX/G1/ETX/99) remain out of scope** — no equally rigorous, byte-level source was found for any of them in this same search pass (the batch header's own STX+"G1"+10-digit-reference-number prefix IS confirmed, but its trailer's exact field layout is not, from any free source found) — named, deferred, not attempted.

### B3 (Rebill) — a pure duplicate of B1, confirmed not assumed
The payer sheet's own field-level table for the Transaction Code field (104-A4) lists its value as literally `"B1, B3"` in the SAME row, and every section heading through the entire combined B1/B3 request+response template is explicitly titled `(B1/B3)`, never split into a B3-only section — B1 and B3 are, per this real payer's own implementation, structurally identical end to end. `transactions/B3_request.json`/`B3_response.json` are intentional structural duplicates of `B1_request.json`/`B1_response.json` with only the `code` field changed — not a guess, a directly-sourced "one shared template" fact.

### B2 (Reversal) — genuinely narrower, and a real schema-widening it required
The SAME payer sheet's own dedicated "Request/Response Claim Reversal (B2)" sections show a materially different, narrower shape: **request** = Transaction Header + Insurance Segment (04) + Claim Segment (07) only — no Patient, Prescriber, Pharmacy Provider, or Pricing segment at all; the Claim Segment itself carries ONLY 455-EM (Prescription/Service Reference Number Qualifier) + 402-D2 (Prescription/Service Reference Number), Mandatory — every other Claim field is excluded from B2's own template (the payer sheet's own legend: excluded = Not Used). **Response** (all 3 variants — Approved, Accepted/Rejected, Rejected/Rejected) = Response Transaction Header + Response Status (21) + Response Claim (22) only — no Response Message, no Response Pricing (a reversal has nothing to price).

**A real, necessary schema widening** (`segments/Claim.json`): B2 reuses the SAME shared `Claim` segment definition B1/B3 use, but this engine's `TelecomFieldDef.Required` is a GLOBAL per-segment flag, not per-transaction — B1's own Claim segment had `productServiceIdQualifier`/`productServiceId`/`quantityDispensed`/`fillNumber`/`daysSupply`/`datePrescriptionWritten` all `required: true` (correct for B1), which would have made every legitimate B2 transmission fail validation for omitting fields B2 never carries. Widened all 6 to `required: false` — a **purely permissive** change, the same class of fix already established for EDI X12's CL1/SV1/SV2/TRN/UM (CLAUDE.md's own "Purely permissive widenings — zero behavior change for every existing consumer, which already supplies these segments in every real fixture"): every real B1 fixture already populates these fields in practice, so B1's own validation behavior is unchanged; only B2's genuinely narrower real claim segment now validates correctly. `prescriptionReferenceNumber` (D2) stays required — needed by both B1 and B2.

### E1 (Eligibility Verification) — no claim/pricing concept at all, and a genuinely narrower response than the base standard allows
Confirmed directly from the same payer sheet: E1's **request** = Transaction Header + Insurance Segment (04, including Patient Relationship Code 306-C6 — already modeled in `Insurance.json`'s own `patientRelationshipCode` field from Phase 1) + Patient Segment (01) + Prescriber Segment (03) — no Claim, Pricing, or Pharmacy Provider segment, and **no transaction-group/repeating-claim concept whatsoever** (`transactionGroupSegments: []`) — a real structural fact this engine's own empty-list code path in `validator.go`/`parser.go` already handles correctly with zero engine changes (confirmed by reading both files directly before writing the schema, not assumed safe). **Response** (all 3 variants) = Response Transaction Header + Response Status (21) ONLY — no Response Message, Response Insurance, Response Patient, Response Claim, or Response Pricing segment appears anywhere in any of this real payer's own 3 response variants.

**A genuine source-extraction ambiguity, resolved conservatively per the "no guesswork" mandate**: the raw `pdftotext -layout` output for E1's request showed a duplicated block between the Insurance and Prescriber field tables — one copy mislabeled "Pharmacy Segment Questions" but containing IDENTICAL Prescriber-shaped content (Provider ID Qualifier/Provider ID/Prescriber Last Name/First Name) to the immediately-following, correctly-labeled Prescriber block. Recognized as a known multi-column-PDF-to-text reconstruction artifact (a stray duplicated table), not a second, genuine Pharmacy segment for E1 — modeled as ONE Prescriber segment rather than fabricating a distinct Pharmacy segment from what is most plausibly a transcription duplicate.

**Named, sourced (not guessed) simplification**: the base D.0 standard's own richer Response Insurance (25)/Response Patient (29) segments exist in this engine's own segment-library reference sources (`apiv/dzero`'s `25_response_insurance.rb`/`29_response_patient.rb`) and could in principle carry real coverage-detail content on a fuller E1 response — but this specific real payer's own implementation doesn't use them, and their exact field dictionaries haven't been sourced with this schema's own established rigor. Left out entirely rather than fabricated; a richer E1 response is a named, deferred future extension.

### Engine-only this round — no FHIR mapping, no dedicated pipeline template
Matching the established "prove the engine first, FHIR mapping is a separate, later round" precedent (835's own engine-then-FHIR split; NCPDP SCRIPT's Status/Error/Verify/GetMessage shipping engine-only). B2/B3/E1 are real, tested, and live in the schema browser API, but have no `fhir.build`/`payload.builder` OOB template and no synchronous endpoint wiring yet (`sync_pharmacy_claim_controller.go` stays deliberately B1-only — widening it to other transaction codes is a real, separate design decision, e.g. a `:transactionCode` route param, not attempted this round). `NCPDP_TELECOM_TRANSACTION_CODES` in `NCPDPTelecomStepBuilder.js` was widened to `['B1', 'B2', 'B3', 'E1']` so the existing `ncpdptelecom.parse`/`validate`/`build`/`map_to_canonical` steps can already target any of the 4 codes — only the FHIR-mapping/sync layers remain B1-specific.

### Verification
Go: 8 new tests in `ncpdptelecom/phase2_transactions_roundtrip_test.go` (B2 request + response [approved/rejected subtests], B3 request/response as structural duplicates of B1's own fixtures, E1 request + response [approved/rejected subtests]) — all pass, run via a throwaway `docker build --target gobuilder` image (schemas/tests bind-mounted, image removed after use). Full `ncpdptelecom`/`ncpdptelecom/validator` suites re-run with zero regressions (the pre-existing 18-scenario generated sample battery, real cosyte fixtures, and B1's own round-trip tests all still pass unchanged, confirming the Claim.json widening is genuinely behavior-preserving for B1). `services/executors/transform`'s NCPDP Telecom executor tests also re-run with zero regressions.

Full-stack: real `docker-compose build app` (clean) + `docker-compose up -d --force-recreate app`, `GET /api/ncpdp-telecom/schema/transactions` (via the Node proxy, port 3000 — the Go backend's own port 8080 correctly rejects direct access) confirmed all 8 transaction entries (B1/B2/B3/E1 × request/response) live with the exact segment lists this section documents.

### Key Files
- Schema widening: `ncpdptelecom/schemas/telecom_d0/segments/Claim.json` (6 fields relaxed to `required: false`, sourceRefs documenting the B2 payer-sheet finding)
- New transaction definitions: `ncpdptelecom/schemas/telecom_d0/transactions/{B2,B3,E1}_{request,response}.json`, `manifest.json` (registration)
- UI: `public/js/pipeline/components/NCPDPTelecomStepBuilder.js` (`NCPDP_TELECOM_TRANSACTION_CODES` widened)
- Tests: `ncpdptelecom/phase2_transactions_roundtrip_test.go`

### Named future phases
E2, D0/D1 (Prior Authorization), N1-N3 (Information Reporting), C1-C3 (Controlled Substance Reporting), S1-S3 (Service Billing), and the NCPDP batch standard (STX/G1/ETX/99 wrapping) remain unsourced-to-this-schema's-own-bar and unattempted. NCPDP SCRIPT's own `RxHistoryRequest`/`RxHistoryResponse` was investigated in this same round and found FULLY buildable — real, complete field-level structure confirmed directly from the NIST SCRIPT-10.6 XSD (`HistoryRequestPatientType`, `OptionalPharmacyType`, `OptionalPrescriberType`, `BenefitsCoordinationRequestType` request-side; `HistoryResponseType`/`ApprovedType`/`DeniedType`, `HistoryPrescriberType`, `HistoryResponsePatientType`, `BenefitsCoordinationResponseType` response-side; `MandatoryPatientNameType` confirmed identical to this engine's own existing `Name` group) — genuinely not yet implemented, since modeling `PatientIDType`/`PayerIDType` (each a 25-option identifier choice) requires picking a real-world-common subset the same way `PharmacyIdentification`/`PrescriberIdentification` already did for NCPDID/NPI, a judgment call deliberately left for its own focused follow-up rather than rushed. `Resupply` and `Census` (also NIST-XSD-sourced) need substantially more new group types (Facility, Allergy, DiagnosisGeneral, Observation, Supervisor) not yet scoped at the same field level. `RxTransferRequest/Response/Confirm`, `REMS`, `DrugAdministration`, `Recertification` remain genuinely unsourced — no structural detail exists in either of this engine's own established reference sources (`cosyte/ncpdp`, the NIST XSD) — and stay deferred, not guessed.

## NCPDP Outbound Direction — FHIR to SCRIPT/D.0 (September 2026)

### The gap this closes
Every NCPDP OOB template built before this round (10 total — SCRIPT's V258-V265, D.0's V266-V267) goes wire-format-IN → FHIR-OUT, terminating at a `sink_outbound` (a no-op store, confirmed by directly inspecting every template's own `connector.outbound` step config before writing this one). None of them prove ezHealthKonnect can go the OTHER direction — build a real NCPDP message from other source data and actually send it somewhere. Asked directly by the user ("I need the app to be at a position to allow a user to build NCPDP interface inbound as well as outbound, are we at that stage?") — the honest answer at the time was no; this round closes that gap for both formats, with the SAME "no guesswork" rigor applied throughout this project's NCPDP work.

### Why FHIR, and why these two resources
The user confirmed the realistic trigger is FHIR (not raw HL7/CSV/DB — though see the "genuinely source-agnostic" note below, this still holds for any of those): a doctor writes a prescription in the EHR (a `MedicationRequest` is created) → build a NewRx and send it to the pharmacy; a drug gets dispensed at a pharmacy (a `MedicationDispense` is created — the SAME resource this codebase's own inbound RxFill mapping already targets) → build a B1 claim and send it to a payer/switch. This mirrors the "reply within the same interface" vs. "independent trigger from other data" distinction discussed with the user directly — both of these are the latter: a genuinely new business event in another system, not a reply to an inbound wire message.

**Confirmed, not assumed, before writing any code**: for an `http_fhir_inbound`-triggered pipeline, the FHIR resource's own fields land FLAT AT ROOT for the first step (`processing/engine_message_processor.go`'s own `sourceType == "http_fhir"` handling: "Build a ParserResult so the transformation pipeline receives FHIR data at root... identical envelope contract to HL7 messages") — and `map_to_canonical`'s `sourcePath` resolution falls through to plain JSON-path resolution for a bare path with no resource-type prefix (verified directly in `field_utils.go`'s `GetFieldValue`/`DetectPathType`/`resolveJSONPathValue`).

### Honest scope, stated up front (not fabricated FHIR structure)
A bare `MedicationRequest`/`MedicationDispense` resource doesn't inline full patient/prescriber/pharmacy demographics or insurance/coverage data — those live on separate `Patient`/`Practitioner`/`Organization`/`Coverage` resources in a real system. Rather than invent structure that isn't there:
- Names come from `Reference.display` (a real, spec-legal FHIR field meant exactly for "human-readable identification without the full resource"), split into first/last via a small, clearly-labeled convenience in the derive script — not a fabricated FHIR shape.
- NPIs come from `Reference.identifier.value` — the same logical-reference pattern already established in this codebase for EDI 837's `Claim.careTeam[].provider`.
- D.0's `Insurance.cardholderId` is approximated from `subject.identifier.value` (the patient's own identifier, e.g. an MRN) — a real but imperfect proxy, documented as such, not presented as a true cardholder-ID lookup.
- Payer-enrollment-level constants no FHIR resource could ever carry (SCRIPT's `to`/`from` mailbox IDs; D.0's `binNumber`/`serviceProviderIdQualifier`) are literal config placeholders the deploying user replaces — the same "config fills in what data doesn't supply" precedent as `CdaCustodianConfig`/EDI's ISA-GS config.

### Genuinely source-agnostic, not just FHIR — confirmed by reading the code
Both `ncpdp.map_to_canonical` and `ncpdptelecom.map_to_canonical` resolve every field via `executors.GetFieldValue(row, f.SourcePath)` — the SAME format-agnostic primitive (`services/executors/field_utils.go`) used throughout this platform for HL7/FHIR/JSON, with zero format-specific logic in either executor. The FHIR-triggered templates below are ONE proven demonstration of the mechanism, not a FHIR-only capability — a user can swap the `connector.inbound` type (HL7, CSV via `enrichment.file_parser`, a DB poller) and adjust `sourcePath` values to build the exact same outbound NewRx/B1 from whatever source system they actually have.

### A real, generic engine bug found and fixed
`ncpdptelecom.Format()` (`ncpdptelecom/datatypes.go`) only accepted native Go numeric types (`float64`/`int`/etc.) for N/R/RO fields, never a numeric-looking STRING — but `map_to_canonical`'s entire mapping mechanism is string-based by design (`sourcePath`/`literalValue` resolution always produces strings, matching `edi.map_to_canonical`/`ncpdp.map_to_canonical`'s own convention). Every prior D.0 Go test built its canonical JSON directly with real `float64` values (parse→build round trips, hand-built fixtures) — this outbound work was the FIRST thing in this codebase to actually exercise `ncpdptelecom.build` via the string-based mapping layer, and it failed immediately on `transactionCount: "1"`. Fixed generically in `asFloat()` (parses a numeric string via `strconv.ParseFloat`), not routed around — a real, previously-latent gap now closed for any future D.0 mapping too.

### A real, pre-existing platform bug found (unrelated to NCPDP) — `POST /api/interfaces` silently failed to provision message tables
Discovered during full-stack verification, not by inspection: `services/InterfaceTableManager.js` exports an already-instantiated SINGLETON (`module.exports = new InterfaceTableManager()`), but `controllers/interfacesController.js` called `new InterfaceTableManager()` on that singleton at BOTH its interface-creation call site (line ~357) and its interface-deletion call site (line ~941) — a `TypeError: InterfaceTableManager is not a constructor`, silently swallowed by an enclosing `try/catch` that only logs `❌ Failed to create storage for interface ...` and continues, leaving the interface with NO message table at all. Confirmed via a repo-wide grep that every OTHER call site (`MessageController.js`, `DeliveryService.js`, `InterfaceTableMaintenanceService.js`, the service's own unit test) already used the required value directly, with no `new` — only these 2 call sites in this one file had the bug. This means **every interface ever created via the generic `POST /api/interfaces` endpoint** (used by "Use Template", and likely the base interface-creation UI too) has been silently failing to provision its own message table since whenever `InterfaceTableManager.js` was last changed to export a singleton instead of a class. Fixed by removing `new` at both call sites, matching the already-correct convention everywhere else. A genuinely valuable, previously-undiscovered bug this NCPDP full-stack verification surfaced as a side effect — not found by inspecting NCPDP code at all.

### An operational gotcha worth knowing (not a code bug — a real test-sequencing lesson)
Deactivating/soft-deleting an interface via `POST .../deactivate` DURING an app restart window can fail with "Go backend service unavailable" (the Go process briefly down) — if that happens, the connector's own goroutine is NEVER actually stopped (only the DB row gets soft-deleted via the Node-only path), leaving an ORPHANED listener still bound to its port. A LATER interface trying to bind that SAME port (e.g., `http_fhir_inbound` on a fixed port like 9610) then fails silently (logged as `❌ HTTP FHIR Inbound: server error: listen tcp :9610: bind: address already in use`, but activation itself still reports success) — meaning messages posted to that port are serviced by the stale, orphaned OLD connector instead of the new one, producing a confusing "accepted but never processed" symptom with zero errors anywhere in the new interface's own status. The only real fix once this happens is a full app restart (clears all in-process connector goroutines) before retrying. Not fixed generically this round (a real, separate, larger scoped item — ensuring deactivation is retried/guaranteed rather than best-effort) — named here so a future full-stack test recognizes the symptom immediately instead of re-debugging it from scratch.

### Verification
Go: `services/executors/transform/newrx_outbound_test.go` (`TestNewRxOutbound_FromFHIRMedicationRequest_BuildsValidNewRxXML`) and `services/executors/transform/b1_outbound_test.go` (`TestB1Outbound_FromFHIRMedicationDispense_BuildsValidB1Transmission`) each chain the REAL executors (`enrichment.script` → `ncpdp(telecom).map_to_canonical` → `ncpdp(telecom).build`) against a real-shaped, self-authored FHIR resource, asserting the built document re-parses cleanly through `ncpdp.ParseMessage`/`ncpdptelecom.ParseTransmission` — the same bar every other NCPDP transaction type in this codebase proves itself against. Full `ncpdp`/`ncpdptelecom`/`services/executors/transform`/broader `services/...` suites re-run with zero regressions (confirmed via a full-repo-mounted Docker test run after an initial narrow-bind-mount run produced a false-positive failure set, matching this project's own already-documented "bind-mount omission" gotcha).

Full-stack (genuinely end-to-end, not Test Pipeline's dry-run — confirmed directly that `IsTestMode` skips the real `Send()` call, so this HAD to be a real, non-test-mode message): two new OOB templates (`V268`/`V269`) created via the exact same 3-call sequence the UI's own "Use Template" flow uses (`POST /api/interface-templates/:id/use` → `POST /api/interfaces` → `POST /api/pipelines`, with the `connector.outbound` step's `url` pointed at a real throwaway HTTP receiver on the app's own docker network), activated, and driven with real POSTed FHIR resources from a container on that same network. Result: a real, correctly-populated NewRx XML (patient/prescriber names, NPI, drug/NDC, quantity, sig, date all correctly transformed from the source FHIR) and a real, correctly-populated B1 transmission (BIN, service provider ID, date-of-service in CCYYMMDD, Insurance/Claim segments) both arrived at the real receiver over a real network hop. Interfaces deactivated and soft-deleted, throwaway receiver container and image removed after verification.

### Named future work
No dedicated OOB template exists yet for CancelRx/RxChangeRequest/other SCRIPT outbound transaction types, or for D.0's B2 (reversal-outbound)/E1 (eligibility-check-outbound) — this round proves the mechanism on the two highest-value cases (NewRx, B1) per the user's own stated priority; extending to the others is a pure config/migration exercise using the same `enrichment.script` → `map_to_canonical` → `build` → `connector.outbound` shape, not new engine work. The deactivation-doesn't-guarantee-connector-stop gotcha (above) is a real, separate, unstarted fix.

### Key Files
- Engine fix: `ncpdptelecom/datatypes.go` (`asFloat` now accepts numeric strings)
- Platform bug fix (unrelated to NCPDP, found via this work): `controllers/interfacesController.js` (2 call sites, removed erroneous `new InterfaceTableManager()`)
- New transform: `services/executors/transform/canonical_value_transforms.go` (`datetime_to_ncpdp_date`)
- Go tests (source of truth for both migrations): `services/executors/transform/newrx_outbound_test.go`, `services/executors/transform/b1_outbound_test.go`
- Migrations: `database/migrations/V268__NCPDP_NewRx_FHIR_To_SCRIPT_Outbound_Template.sql`, `V269__NCPDP_B1_FHIR_To_Telecom_Outbound_Template.sql`

## NCPDP Outbound — Closing the Comprehensiveness Gaps (September 2026)

### Why this round exists
Directly asked, twice: "did you do a fullstack end to end comprehensive testing?" The honest answer to the first pass was no — it was genuinely full-stack (real interface, real network delivery) but not comprehensive (no browser/UI verification, no validate step, an honest B1 gap never actually checked). This round closes every gap named in that answer.

### Gap 1 — missing `validate` step, closed, and it immediately did its job
Neither V268 nor V269 originally had a `*.validate` step between `map_to_canonical` and `build`, unlike every inbound-direction template. Added `ncpdp.validate`/`ncpdptelecom.validate` at sequence 35 in both (non-blocking, `required: false`, matching this codebase's own "validation is advisory" precedent) — zero new engine code, since both validate executors already accept `map_to_canonical`'s own output shape directly (`resolveNCPDPParseResult`/`resolveNCPDPTelecomParseResult` recognize a map with `transactionType`/`transactionCode` keys and validate it without re-parsing).

**Adding it immediately surfaced a real, honest gap**: the B1 outbound mapping validated `valid=false` — missing `Patient`, `PharmacyProvider`, and `Pricing` segments entirely. Two of the three were real, fixable omissions (the derive script simply never extracted `subject.display`/reused `pharmacy_npi` into those segments) — fixed by adding `Patient` (split from `subject.display`, the same technique already used for NewRx) and `PharmacyProvider` (reusing the already-derived `pharmacy_npi`). **Pricing is a genuine, permanent, named gap**: `ingredientCostSubmitted`/`usualAndCustomaryCharge`/`grossAmountDue` are the pharmacy's own point-of-sale cost data, which `MedicationDispense` does not model at all — fabricating a `$0.00` placeholder would be actively misleading to a real payer (indistinguishable from a genuinely free claim), not a harmless default. Left honestly absent; the Go test and Playwright spec both assert this is the ONLY remaining validation issue, not that it's silently ignored.

### Gap 2 — no browser verification, closed, and it found a real bug the "full-stack via API" pass couldn't have
`tests/playwright/ncpdp-outbound-e2e.spec.js` drives the real "Use Template" click path for both templates end to end: templates gallery → configure modal → redirect to `pipeline-builder.html` → confirms 6 canvas nodes → opens all 3 non-trivial step-config panels (`enrichment.script`, `map_to_canonical`, `validate`) to confirm they render without error → runs a real "Test Pipeline" call with a realistic FHIR payload → asserts on the real built output.

**A real bug this found, invisible to every prior test in this whole body of work**: the derive scripts' own `input.message || input` fallback (added specifically to handle the well-documented "prior step's output is nested under `message` for a later step" gotcha) is WRONG for this specific position in the pipeline. The Test Pipeline harness (`controllers/pipeline_test_controller.go`) unconditionally adds a `message: {raw_content, detected_format}` key to `inputData` for EVERY format, even for the very first step — a genuinely different envelope shape than the real engine's own `http_fhir_inbound` path (which adds no `message` key at all for the first step). Since `input.message` was truthy but held the WRONG object (no FHIR fields at all), the derive script's own fallback picked the wrong branch and produced entirely empty output — with no error anywhere, since the script has no way to know it received the wrong shape. This never showed up in the Go-level tests (which construct `inputData` by hand, with no `message` key) or the earlier API-replicated full-stack pass (which drives the real `http_fhir_inbound` connector, which also never adds a `message` key for the first step) — only the REAL Test Pipeline harness's own third, different envelope shape exposed it.

Fixed generically, not patched around: both derive scripts now check `input.resourceType` first (every real FHIR resource always carries this field) to decide whether the resource landed at root, falling back to `input.message.resourceType` only if root doesn't have it, and finally to bare `input` as a last resort. This is robust across all three envelope shapes this codebase can hand a first-position `enrichment.script` step (raw engine, Test Pipeline harness, and any hypothetical later-step position) — a real, generalizable lesson for the NEXT enrichment.script step positioned right after any inbound connector, not just these two templates.

**A second, smaller lesson**: the Test Pipeline endpoint's own step-output snapshot (like the real pipeline engine's) is passed through `NormalizeStepOutput`, snake-casing `ncpdpScript`/`ncpdpTelecom`/`ncpdpValidation`/`telecomValidation` to `ncpdp_script`/`ncpdp_telecom`/`ncpdp_validation`/`telecom_validation` — the Playwright spec checks both forms defensively rather than hardcoding a guess.

### Verification
Go: `newrx_outbound_test.go`/`b1_outbound_test.go` extended to chain `enrichment.script → map_to_canonical → validate → build`, both asserting the validate step's own real output (NewRx: `valid=true`, zero issues; B1: `assertOnlyIssuePaths` confirms `TransactionGroups[0].Pricing` is the ONLY issue, catching both under- and over-reporting as a regression). Full `ncpdp`/`ncpdptelecom`/`services/...` suite (every subpackage, not just the touched ones) re-run clean — zero regressions from the `resourceType`-based envelope-detection fix, the Patient/PharmacyProvider mapping additions, or the validate step insertion.

Full-stack + browser: both migrations re-applied live (`flyway repair` + direct `psql` re-apply, the same "safe only because these are minutes-old, same-session, unreleased-beyond-this-repo edits" precedent already established for V237/V238/V260/V262/V264) — confirmed via `psql` both templates now show 6 execution groups. `npx playwright test tests/playwright/ncpdp-outbound-e2e.spec.js --project=chromium` passes both tests cleanly (3/3 including the shared auth setup), zero console errors, real built NewRx XML and B1 transmission asserted by content. All 9 interfaces the Playwright runs created (including retry attempts) deactivated/soft-deleted via the real API afterward.

### Key Files
- Derive-script fix (generic envelope-shape detection): `services/executors/transform/newrx_outbound_test.go`, `services/executors/transform/b1_outbound_test.go` (both `derive*OutboundFieldsScript` functions)
