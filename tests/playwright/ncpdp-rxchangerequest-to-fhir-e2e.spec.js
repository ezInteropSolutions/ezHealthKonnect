// tests/playwright/ncpdp-rxchangerequest-to-fhir-e2e.spec.js
// V261's own OOB template (NCPDP SCRIPT RxChangeRequest -> FHIR), driven
// through the REAL "Use Template" click path AND a REAL "Test Pipeline" run
// against the self-authored RxChangeRequest sample.
...
test('NCPDP SCRIPT RxChangeRequest to FHIR template: Use Template renders 10 steps, then Test Pipeline against the self-authored RxChangeRequest sample produces a clean-validating Bundle', async ({ page }) => {
    const consoleErrors = [];
    page.on('console', msg => {
    ...
    const card = page.locator('.tg-card', { hasText: 'NCPDP SCRIPT RxChangeRequest to FHIR' });
    await expect(card, 'Templates gallery should list the NCPDP SCRIPT RxChangeRequest to FHIR template').toBeVisible({ timeout: 5000 });
    await card.locator('.tg-use-btn').click();

    await page.waitForSelector('#tgConfigureModal', { state: 'visible', timeout: 5000 });
    await page.fill('#tcf_name', `PW NCPDP-RxChangeRequest-FHIR Test ${Date.now()}`);
    ...
    await page.locator('.flowchart-step-node', { hasText: 'Parse Rx Change Request' }).click();
    ...
    const sample = fs.readFileSync(
        path.join(__dirname, '..', '..', 'ncpdp', 'testdata', 'self_authored', 'rx_change_request_sample.xml'),
        'utf8'
    );
    ...
    const assembleStep = testOutput.steps?.assemble_rx_change_request_fhir_bundle;
    expect(assembleStep?.step_output?.payload, 'Assemble Rx Change Request FHIR Bundle should produce a payload string').toBeTruthy();
    ...
    expect(medReq.status, 'MedicationRequest.status should be draft for a pharmacy-proposed change').toBe('draft');
    expect(medReq.intent, 'MedicationRequest.intent should be proposal for a pharmacy-proposed change').toBe('proposal');
    expect(medReq.identifier[0].value).toBe('REQ-0002');
    expect(medReq.medicationCodeableConcept.text).toBe('Ondansetron 8 mg Tab Disintegrating');
    expect(medReq.subject.reference).toBe(patientEntry.fullUrl);
    expect(medReq.requester.reference).toBe(practitionerEntry.fullUrl);
