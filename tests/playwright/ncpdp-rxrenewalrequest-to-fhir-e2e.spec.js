// tests/playwright/ncpdp-rxrenewalrequest-to-fhir-e2e.spec.js
// V263's own OOB template (NCPDP SCRIPT RxRenewalRequest -> FHIR), driven
// through the REAL "Use Template" click path AND a REAL "Test Pipeline" run
// against the self-authored RxRenewalRequest sample.
...
test('NCPDP SCRIPT RxRenewalRequest to FHIR template: Use Template renders 10 steps, then Test Pipeline against the self-authored RxRenewalRequest sample produces a clean-validating Bundle', async ({ page }) => {
    const consoleErrors = [];
    page.on('console', msg => {
    ...
    const card = page.locator('.tg-card', { hasText: 'NCPDP SCRIPT RxRenewalRequest to FHIR' });
    await expect(card, 'Templates gallery should list the NCPDP SCRIPT RxRenewalRequest to FHIR template').toBeVisible({ timeout: 5000 });
    await card.locator('.tg-use-btn').click();

    await page.waitForSelector('#tgConfigureModal', { state: 'visible', timeout: 5000 });
    await page.fill('#tcf_name', `PW NCPDP-RxRenewalRequest-FHIR Test ${Date.now()}`);
    ...
    await page.locator('.flowchart-step-node', { hasText: 'Parse Rx Renewal Request' }).click();
    ...
    const sample = fs.readFileSync(
        path.join(__dirname, '..', '..', 'ncpdp', 'testdata', 'self_authored', 'rx_renewal_request_sample.xml'),
        'utf8'
    );
    ...
    const assembleStep = testOutput.steps?.assemble_rx_renewal_request_fhir_bundle;
    expect(assembleStep?.step_output?.payload, 'Assemble Rx Renewal Request FHIR Bundle should produce a payload string').toBeTruthy();
    ...
    expect(medReq.status, 'MedicationRequest.status should be active for a renewal request').toBe('active');
    expect(medReq.intent).toBe('order');
    expect(medReq.identifier[0].value).toBe('REQ-0003');
