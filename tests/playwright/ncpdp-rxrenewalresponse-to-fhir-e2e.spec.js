// tests/playwright/ncpdp-rxrenewalresponse-to-fhir-e2e.spec.js
// V264's own OOB template (NCPDP SCRIPT RxRenewalResponse -> FHIR Task),
// driven through the REAL "Use Template" click path AND a REAL "Test
// Pipeline" run against the self-authored Approved-outcome sample.
...
test('NCPDP SCRIPT RxRenewalResponse to FHIR template: Use Template renders 7 steps, then Test Pipeline against the self-authored Approved sample produces a clean-validating Task', async ({ page }) => {
    const consoleErrors = [];
    page.on('console', msg => {
    ...
    const card = page.locator('.tg-card', { hasText: 'NCPDP SCRIPT RxRenewalResponse to FHIR' });
    await expect(card, 'Templates gallery should list the NCPDP SCRIPT RxRenewalResponse to FHIR template').toBeVisible({ timeout: 5000 });
    await card.locator('.tg-use-btn').click();

    await page.waitForSelector('#tgConfigureModal', { state: 'visible', timeout: 5000 });
    await page.fill('#tcf_name', `PW NCPDP-RxRenewalResponse-FHIR Test ${Date.now()}`);
    ...
    await page.locator('.flowchart-step-node', { hasText: 'Parse Rx Renewal Response' }).click();
    ...
    await page.locator('.flowchart-step-node', { hasText: 'Build Rx Renewal Response Task' }).click();
    await page.waitForSelector('#stepPropertiesModal', { state: 'visible', timeout: 5000 });
    await expect(page.locator('#fbbResourceType')).toHaveValue('Task');
    await closeStepModal(page);

    const sample = fs.readFileSync(
        path.join(__dirname, '..', '..', 'ncpdp', 'testdata', 'self_authored', 'rx_renewal_response_approved_sample.xml'),
        'utf8'
    );
    ...
    const assembleStep = testOutput.steps?.assemble_rx_renewal_response_fhir_bundle;
    expect(assembleStep?.step_output?.payload, 'Assemble Rx Renewal Response FHIR Bundle should produce a payload string').toBeTruthy();
    ...
    expect(task.status, 'Task.status should be completed for an Approved outcome').toBe('completed');
    expect(task.businessStatus.coding[0].code).toBe('Approved');
    expect(task.note[0].text).toBe('Renewal approved as written');
    expect(task.identifier[0].value).toBe('REQ-0003');
