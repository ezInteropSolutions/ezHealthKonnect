...
-- NOTE: sample_parsed_messages is created in V37. This migration is wrapped in
-- IF EXISTS guards so it's a no-op on fresh installs (columns are included in V37).

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sample_parsed_messages') THEN
        -- Add new columns
