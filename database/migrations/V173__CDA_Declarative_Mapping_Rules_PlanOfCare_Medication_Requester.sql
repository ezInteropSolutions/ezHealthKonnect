-- V173__CDA_Declarative_Mapping_Rules_PlanOfCare_Medication_Requester.sql
--
-- Re-seeds the MedicationRequest row in all 4 Plan-of-Care section aliases
-- (carePlan, planOfCare, assessmentAndPlan, planOfTreatment -- previously
-- seeded by V158, then V171) to add the same 2-tier PractitionerRole-
-- emitting requester V172 added to the Medications section's
-- MedicationRequest. PlanOfCareMappingRules()'s substanceAdministration
-- branch reuses medicationRequestFields() verbatim (see that function's own
-- doc comment), so all 4 of these rows drift simultaneously from the same
-- Go-side change. Supersedes V171's coverage entirely.
--
-- See declarative_oob_rules_migration_v173_test.go for the drift guard.
