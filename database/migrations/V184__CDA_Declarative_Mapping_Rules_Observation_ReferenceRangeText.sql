-- V184__CDA_Declarative_Mapping_Rules_Observation_ReferenceRangeText.sql
--
-- Re-seeds all 6 observationRule()-based sections (vitalSigns, results,
-- labResults, functionalStatus, mentalStatus, socialHistory) to add a new
-- referenceRange[0].text row, sourced from a new CDAEntry.ReferenceRangeText
-- field (cda/document/types.go + entry_parser.go, this same change) that
-- parses <referenceRange><observationRange><text>.
--
-- Real gap found auditing Results (Ascension Wisconsin sample, CCD_06_24_20267.xml):
-- all 46 lab Observations in that file's Results section (CBC, Chem panel,
-- differential) carry a real free-text reference range (e.g.
-- "4.0 - 10.0 Thou/uL") that was parsed nowhere before this -- confirmed via
-- a real engine run that every one came back with referenceRange=nil. A FHIR
-- transform for this already existed (cda_reference_range_to_fhir) but
-- expects a structured low/high shape that 0/4 corpus files actually use;
-- only the free-text shape is evidenced, so this row is a plain string
-- passthrough (no transform), same idiom as the note[0].text rows elsewhere
-- in this file.
--
-- This row is shared by all 6 observationRule() callers, so all 6 drift
-- simultaneously from this one Go-side change -- same multi-section re-seed
-- shape as V169/V178. Supersedes V178 for vitalSigns/results/labResults/
-- socialHistory, V179 for functionalStatus, V180 for mentalStatus -- see
-- services/cda_fhir/declarative_oob_rules.go's observationRule() for the
-- single source of truth this JSON is hand-synced to, and
-- declarative_oob_rules_migration_v184_test.go for the drift guard.
