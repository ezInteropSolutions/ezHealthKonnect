...
--     script's output) to steps.stamp_bundle_profile.step_output.fhirBundle
--     -- "fhirBundle", not "pas_bundle": models.OutputNormalizer's own
--     preserve-list only protects "result"/"fhirBundle"/"fhirResource" from
--     recursive snake-casing, so any other key name (the original
--     "pas_bundle") has its entire nested FHIR content mangled
--     (resourceType -> resource_type, etc.) once captured into this step's
--     own steps.<alias>.step_output snapshot -- found via a real browser
--     Test Pipeline run, not by static reading.
--
...
SET pipeline_config = jsonb_set(
    ...
    (
        ...
        FROM (
                ...
                CASE
                    ...
                    WHEN g -> 'steps' -> 0 ->> 'step_alias' = 'validate_pas_bundle' THEN
                        jsonb_set(
                            ...
                            '"steps.stamp_bundle_profile.step_output.fhirBundle"'::jsonb
                        )
            ...
            SELECT jsonb_array_elements($groups$[{"sequence":95,"label":"Zone 2 — Derive Computed Fields","description":"Pre-built and locked. Pure data derivation only (gender whitelist, priority mapping, diagnosis code normalization, service date defaults, generated IDs) — no FHIR resource-shape or profile knowledge. Every fhir.build step below reads its output via steps.derive_pas_computed_fields.step_output._pas_derived.* (snake_case field names).","steps":[{"step_name":"Derive PAS Computed Fields","step_alias":"derive_pas_fields","step_type":"enrichment.script","sequence":95,"enabled":true,"required":true,"description":"Computes values fhir.build's declarative config cannot express: gender whitelist (default unknown), urgency->priority mapping (default normal), diagnosis code array normalization, service date defaults, provider full name (Organization.name fallback), and generated claim/bundle IDs.","config":{"_zone":"pas_core","_locked":true,"script":"\n// -- Derive PAS Computed Fields --\n// Pure data derivation only -- no FHIR resource shape or profile knowledge.\n//\n// Zone 1 (field_mapping/pas_envelope_mapping, step_alias \"pas_envelope\",\n// step_name \"Map to PAS Envelope\") writes its own output ONLY into\n// inputData[\"_stepOutput\"] (BaseExecutor.SetStepOutputWithDetails) --\n// executeStepWithContext extracts that into this step's own \"input.steps.<X>.step_output\"\n// snapshot and DELETES \"_stepOutput\" from what actually reaches a later\n// step, so it is NEVER reachable at input._pas_envelope directly (the\n// pre-fix assumption this script made). The snapshot key \"<X>\" is the\n// NORMALIZED STEP NAME (\"Map to PAS Envelope\" -> \"map_to_pas_envelope\"),\n// NOT the step_alias (\"pas_envelope\") -- those two happen to be the same\n// string for this template's own Zone 2 steps (a naming convention, not a\n// guarantee) but genuinely differ for Zone 1, so this is not interchangeable\n// with the alias. Bare input._pas_envelope is kept as a fallback purely for\n// any Go-level test harness that constructs data flat, without the real\n// \"steps\" wrapper.\nvar env = (input.steps && input.steps.map_to_pas_envelope && input.steps.map_to_pas_envelope.step_output && input.steps.map_to_pas_envelope.step_output._pas_envelope) || input._pas_envelope || {};\nvar pat = env.patient || {};\nvar cov = env.coverage || {};\nvar req = env.request || {};\nvar prov = env.provider || {};\n\nvar gender = (pat.gender || \"unknown\").toLowerCase();\nif (gender !== \"male\" && gender !== \"female\" && gender !== \"other\" && gender !== \"unknown\") {\n  gender = \"unknown\";\n}\n\nvar priority = (req.urgency === \"urgent\") ? \"stat\" : \"normal\";\n\nvar diagCodes = req.diagnosis_codes || [];\nif (typeof diagCodes === \"string\") {\n  try { diagCodes = JSON.parse(diagCodes); } catch (e) { diagCodes = [diagCodes]; }\n}\nvar diagnosisCodes = diagCodes.map(function(code) { return { code: String(code) }; });\n\nvar today = new Date().toISOString().substring(0, 10);\nvar serviceStartDate = req.service_start_date || today;\nvar serviceEndDate = req.service_end_date || serviceStartDate;\n\nvar quantity = req.quantity ? Number(req.quantity) : 1;\n\nvar coverageClasses = [];\nif (cov.plan_id) {\n  coverageClasses.push({ type: \"plan\", value: cov.plan_id, name: \"Health Plan\" });\n}\nif (cov.group_number) {\n  coverageClasses.push({ type: \"group\", value: cov.group_number, name: \"\" });\n}\n\nvar ts = new Date().toISOString();\nvar claimId = \"claim-\" + Date.now();\nvar bundleId = \"pas-bundle-\" + Date.now();\n\n// Full name fallback for Organization.name when no explicit organization\n// name is mapped (V91/V212's own guided config has no \"organizationName\"\n// mapping at all) -- provider.firstName/lastName are separate, real fields\n// (see pasFHIRBuilderInput's own doc comment for why), combined here the\n// same way the pre-existing script always did.\nvar providerFullName = ((prov.first_name || \"\") + \" \" + (prov.last_name || \"\")).trim();\n\nreturn ({\n  _pas_derived: {\n    gender: gender,\n    priority: priority,\n    diagnosis_codes: diagnosisCodes,\n    service_start_date: serviceStartDate,\n    service_end_date: serviceEndDate,\n    quantity: quantity,\n    coverage_classes: coverageClasses,\n    claim_id: claimId,\n    bundle_id: bundleId,\n    created_at: ts,\n    provider_full_name: providerFullName\n  }\n});\n"}}]},{"sequence":100,"label":"Zone 2 — Build FHIR Patient","description":"Pre-built and locked. Builds a FHIR R4 Patient resource via the fhir.build engine (config-driven, generic — zero new code per resource type), conforming to the Da Vinci PAS Subscriber profile. Output: message.fhirPatient.","steps":[{"step_name":"Build FHIR Patient","step_alias":"build_patient_fhir","step_type":"fhir.build","sequence":100,"enabled":true,"required":true,"description":"Builds the Patient resource from steps.map_to_pas_envelope.step_output._pas_envelope.patient.* and steps.derive_pas_computed_fields.step_output._pas_derived.gender. Profile: Da Vinci PAS Subscriber.","config":{"fields":[{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.member_id","targetPath":"id"},{"literalValue":"http://hl7.org/fhir/us/davinci-pas/StructureDefinition/profile-subscriber","targetPath":"meta.profile[0]"},{"literalValue":"http://terminology.hl7.org/CodeSystem/v2-0203","targetPath":"identifier[0].type.coding[0].system"},{"literalValue":"MB","targetPath":"identifier[0].type.coding[0].code"},{"literalValue":"Member Number","targetPath":"identifier[0].type.coding[0].display"},{"literalValue":"urn:oid:2.16.840.1.113883.4.6","targetPath":"identifier[0].system"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.member_id","targetPath":"identifier[0].value"},{"literalValue":"official","targetPath":"name[0].use"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.last_name","targetPath":"name[0].family"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.first_name","targetPath":"name[0].given[0]"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.dob","targetPath":"birthDate"},{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.gender","targetPath":"gender"}],"outputField":"message.fhirPatient","profile":"base","resourceType":"Patient","version":"R4","_zone":"pas_core","_locked":true}}]},{"sequence":105,"label":"Zone 2 — Build FHIR Coverage","description":"Pre-built and locked. Builds the FHIR R4 Coverage resource via fhir.build, conforming to the Da Vinci PAS Coverage profile. Output: message.fhirCoverage.","steps":[{"step_name":"Build FHIR Coverage","step_alias":"build_coverage_fhir","step_type":"fhir.build","sequence":105,"enabled":true,"required":true,"description":"Builds the Coverage resource linking Patient to payer. Coverage.class[] (plan/group) is built via repeatingGroups off steps.derive_pas_computed_fields.step_output._pas_derived.coverage_classes (0-2 conditionally-present entries, precomputed upstream). Profile: Da Vinci PAS Coverage.","config":{"fields":[{"literalValue":"coverage-1","targetPath":"id"},{"literalValue":"http://hl7.org/fhir/us/davinci-pas/StructureDefinition/profile-coverage","targetPath":"meta.profile[0]"},{"literalValue":"http://hl7.org/fhir/sid/us-npi","targetPath":"identifier[0].system"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.coverage.payer_id","targetPath":"identifier[0].value"},{"literalValue":"active","targetPath":"status"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.member_id","targetPath":"subscriber.reference","transform":"string_prefix","valueMap":{"prefix":"Patient/"}},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.member_id","targetPath":"subscriberId"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.member_id","targetPath":"beneficiary.reference","transform":"string_prefix","valueMap":{"prefix":"Patient/"}},{"literalValue":"http://terminology.hl7.org/CodeSystem/subscriber-relationship","targetPath":"relationship.coding[0].system"},{"literalValue":"self","targetPath":"relationship.coding[0].code"},{"literalValue":"http://hl7.org/fhir/sid/us-npi","targetPath":"payor[0].identifier.system"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.coverage.payer_id","targetPath":"payor[0].identifier.value"}],"outputField":"message.fhirCoverage","profile":"base","repeatingGroups":[{"fields":[{"literalValue":"http://terminology.hl7.org/CodeSystem/coverage-class","targetPath":"type.coding[0].system"},{"sourcePath":"type","targetPath":"type.coding[0].code"},{"sourcePath":"value","targetPath":"value"},{"sourcePath":"name","targetPath":"name"}],"rowsPath":"steps.derive_pas_computed_fields.step_output._pas_derived.coverage_classes","targetPath":"class"}],"resourceType":"Coverage","version":"R4","_zone":"pas_core","_locked":true}}]},{"sequence":110,"label":"Zone 2 — Build FHIR Practitioner","description":"Pre-built and locked. Builds the FHIR R4 Practitioner resource (ordering clinician) via fhir.build. Output: message.fhirPractitioner.","steps":[{"step_name":"Build FHIR Practitioner","step_alias":"build_practitioner_fhir","step_type":"fhir.build","sequence":110,"enabled":true,"required":true,"description":"Builds the Practitioner resource from steps.map_to_pas_envelope.step_output._pas_envelope.provider.first_name/last_name/npi. Referenced on Claim.item.provider.","config":{"fields":[{"literalValue":"practitioner-1","targetPath":"id"},{"literalValue":"http://hl7.org/fhir/us/davinci-pas/StructureDefinition/profile-practitioner","targetPath":"meta.profile[0]"},{"literalValue":"http://hl7.org/fhir/sid/us-npi","targetPath":"identifier[0].system"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.provider.npi","targetPath":"identifier[0].value"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.provider.last_name","targetPath":"name[0].family"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.provider.first_name","targetPath":"name[0].given[0]"}],"outputField":"message.fhirPractitioner","profile":"base","resourceType":"Practitioner","version":"R4","_zone":"pas_core","_locked":true}}]},{"sequence":115,"label":"Zone 2 — Build FHIR Organization","description":"Pre-built and locked. Builds the FHIR R4 Organization resource (billing/requesting entity) via fhir.build. Output: message.fhirOrganization.","steps":[{"step_name":"Build FHIR Organization","step_alias":"build_organization_fhir","step_type":"fhir.build","sequence":115,"enabled":true,"required":true,"description":"Builds the Organization resource. identifier falls back from facilityNpi to the clinician's own npi (native fallbackPaths, no script needed). name falls back from organizationName to the derived provider full name to a literal default (three-tier fallback).","config":{"fields":[{"literalValue":"organization-1","targetPath":"id"},{"literalValue":"http://hl7.org/fhir/us/davinci-pas/StructureDefinition/profile-requestor","targetPath":"meta.profile[0]"},{"literalValue":"true","targetPath":"active"},{"fallbackPaths":["steps.map_to_pas_envelope.step_output._pas_envelope.provider.npi"],"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.provider.facility_npi","targetPath":"identifier[0].value"},{"literalValue":"http://hl7.org/fhir/sid/us-npi","targetPath":"identifier[0].system"},{"fallbackPaths":["steps.derive_pas_computed_fields.step_output._pas_derived.provider_full_name"],"literalValue":"Requesting Organization","sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.provider.organization_name","targetPath":"name"}],"outputField":"message.fhirOrganization","profile":"base","resourceType":"Organization","version":"R4","_zone":"pas_core","_locked":true}}]},{"sequence":120,"label":"Zone 2 — Build FHIR Claim","description":"Pre-built and locked. Builds the FHIR R4 Claim resource via fhir.build, conforming to the Da Vinci PAS Claim profile. Output: message.fhirClaim.","steps":[{"step_name":"Build FHIR Claim","step_alias":"build_claim_fhir","step_type":"fhir.build","sequence":120,"enabled":true,"required":true,"description":"Builds the Claim resource. Claim.use is a fixed literalValue \"preauthorization\" (IG-required). References to Coverage/Practitioner/Organization use the SAME fixed IDs those resources' own fhir.build steps set (literalValue \"Coverage/coverage-1\" etc.), guaranteeing correlation by construction. diagnosis[] built via repeatingGroups off steps.derive_pas_computed_fields.step_output._pas_derived.diagnosis_codes.","config":{"fields":[{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.claim_id","targetPath":"id"},{"literalValue":"http://hl7.org/fhir/us/davinci-pas/StructureDefinition/profile-claim","targetPath":"meta.profile[0]"},{"literalValue":"urn:ietf:rfc:3986","targetPath":"identifier[0].system"},{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.claim_id","targetPath":"identifier[0].value"},{"literalValue":"active","targetPath":"status"},{"literalValue":"http://terminology.hl7.org/CodeSystem/claim-type","targetPath":"type.coding[0].system"},{"literalValue":"professional","targetPath":"type.coding[0].code"},{"literalValue":"preauthorization","targetPath":"use"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.patient.member_id","targetPath":"patient.reference","transform":"string_prefix","valueMap":{"prefix":"Patient/"}},{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.created_at","targetPath":"created"},{"literalValue":"http://hl7.org/fhir/sid/us-npi","targetPath":"insurer.identifier.system"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.coverage.payer_id","targetPath":"insurer.identifier.value"},{"literalValue":"Organization/organization-1","targetPath":"provider.reference"},{"literalValue":"http://terminology.hl7.org/CodeSystem/processpriority","targetPath":"priority.coding[0].system"},{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.priority","targetPath":"priority.coding[0].code"},{"literalValue":"1","targetPath":"insurance[0].sequence"},{"literalValue":"true","targetPath":"insurance[0].focal"},{"literalValue":"Coverage/coverage-1","targetPath":"insurance[0].coverage.reference"},{"literalValue":"1","targetPath":"item[0].sequence"},{"literalValue":"http://hl7.org/fhir/us/davinci-pas/StructureDefinition/extension-serviceItemRequestedDate","targetPath":"item[0].extension[0].url"},{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.service_start_date","targetPath":"item[0].extension[0].valuePeriod.start"},{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.service_end_date","targetPath":"item[0].extension[0].valuePeriod.end"},{"literalValue":"https://codesystem.x12.org/005010/1365","targetPath":"item[0].category.coding[0].system"},{"literalValue":"1","targetPath":"item[0].category.coding[0].code"},{"literalValue":"Medical Care","targetPath":"item[0].category.coding[0].display"},{"literalValue":"http://www.ama-assn.org/go/cpt","targetPath":"item[0].productOrService.coding[0].system"},{"sourcePath":"steps.map_to_pas_envelope.step_output._pas_envelope.request.service_code","targetPath":"item[0].productOrService.coding[0].code"},{"sourcePath":"steps.derive_pas_computed_fields.step_output._pas_derived.quantity","targetPath":"item[0].quantity.value","transform":"cda_decimal_string_to_number"},{"literalValue":"Practitioner/practitioner-1","targetPath":"item[0].provider.reference"}],"outputField":"message.fhirClaim","profile":"base","repeatingGroups":[{"fields":[{"literalValue":"http://hl7.org/fhir/sid/icd-10-cm","targetPath":"diagnosisCodeableConcept.coding[0].system"},{"sourcePath":"code","targetPath":"diagnosisCodeableConcept.coding[0].code"}],"rowsPath":"steps.derive_pas_computed_fields.step_output._pas_derived.diagnosis_codes","targetPath":"diagnosis"}],"resourceType":"Claim","version":"R4","_zone":"pas_core","_locked":true}}]},{"sequence":130,"label":"Zone 2 — Assemble Da Vinci PAS Request Bundle","description":"Pre-built and locked. Assembles the 5 built resources into a Da Vinci PAS-conformant FHIR R4 Bundle using payload.builder's fhir_bundle mode, which calls fhir/r4/bundle_assembler.go's AssembleEntries to auto-assign urn:uuid: fullUrls and rewrite every internal reference to match — structurally eliminating the fullUrl/reference-form mismatch the previous hand-written assembly script had. Output: steps.assemble_pas_bundle.step_output.payload (a JSON string).","steps":[{"step_name":"Assemble PAS Bundle","step_alias":"assemble_pas_bundle","step_type":"payload.builder","sequence":130,"enabled":true,"required":true,"description":"Lists the 5 built resources as resourcePaths; AssembleEntries handles fullUrl assignment and reference rewriting automatically — every cross-reference (Claim.patient, Claim.insurance[0].coverage, Claim.provider, Claim.item[0].provider, Coverage.subscriber, Coverage.beneficiary) is guaranteed consistent, not hand-coordinated.","config":{"_zone":"pas_core","_locked":true,"mode":"fhir_bundle","fhirBundle":{"bundleType":"collection","resourcePaths":["message.fhirClaim","message.fhirPatient","message.fhirCoverage","message.fhirPractitioner","message.fhirOrganization"]}}}]},{"sequence":135,"label":"Zone 2 — Stamp Bundle Profile","description":"Pre-built and locked. payload.builder's fhir_bundle mode has no config key for Bundle.meta.profile and outputs an already-marshaled JSON string, not a map — this step sets the required Da Vinci PAS request-bundle profile URL and converts the string back to a map for downstream steps (validate_pas_bundle, submit_to_payer). Output: steps.stamp_bundle_profile.step_output.fhirBundle (fhirBundle, not pas_bundle -- models.OutputNormalizer only preserves result/fhirBundle/fhirResource from recursive snake-casing).","steps":[{"step_name":"Stamp Bundle Profile","step_alias":"stamp_bundle_profile","step_type":"enrichment.script","sequence":135,"enabled":true,"required":true,"description":"Sets Bundle.meta.profile to the Da Vinci PAS request-bundle profile URL and re-parses the assembled bundle into a map. Downstream steps (validate_pas_bundle, submit_to_payer) reference steps.stamp_bundle_profile.step_output.fhirBundle instead of the old steps.assemble_pas_bundle.step_output.pas_bundle.","config":{"_zone":"pas_core","_locked":true,"script":"var bundle = JSON.parse(input.steps.assemble_pas_bundle.step_output.payload);\nbundle.meta = { profile: [\"http://hl7.org/fhir/us/davinci-pas/StructureDefinition/profile-pas-request-bundle\"] };\nreturn ({ fhirBundle: bundle });\n"}}]}]$groups$::jsonb) AS grp
        ) fixed
    )
)
